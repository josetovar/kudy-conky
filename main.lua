...
function conky_main(  )
	...
	-- DATE TIME
	-- ################################################################################
	start_x = conky_window.width/40 + (200)
	start_y = 0
	...
	start_y = total_height*(0.40)
	box_width = total_width*(0.42)
	box_height = total_height*(0.25)
	cairo_set_source_rgba(cr, 1,1,1,1)
	...
	start_x = conky_window.width/40 + total_width*(0.01)
	start_y = total_height*(0.65)
	box_width = total_width*(0.42)
	box_height = total_height*(0.25)
	cairo_set_source_rgba(cr, 1,1,1,1)
	...
	options.valign = 1
	options.halign = 0
	x, y = lineText("Uptime", start_x + box_width/18, start_y + box_height/15, box_height/10, "Poiret One", extents, options)
	
	-- uptime value
	options.halign = 1
	options.valign = 0
	options.width = box_width - box_width/18
	x, y = lineText(trim1(conky_parse("${uptime}")), start_x, y + box_height/20 + box_height/20, box_height/20, 'Text Me One', extents, options)
	...
	start_x = conky_window.width/40 + total_width*(0.54)
	start_y = total_height*(0.40)
	box_width = total_width*(0.42)
	box_height = total_height*(0.25)
	cairo_set_source_rgba(cr, 1,1,1,1)
	...
	x, y = lineText('Memory', start_x, start_y + box_height/15, box_height/13, "Poiret One", extents, options)
