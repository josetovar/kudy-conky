...
function conky_main(  )
	...
	start_x = conky_window.width/2 - 260
	start_y = 0
	box_width = total_width/1
	...
	if weather['status'] == 'FILLED' then
		...
		m_summary = split(weather['1_summary'], ",")
		...
		options.valign = 1
	        _, y = lineText('' , start_x + box_width*(0.04), y , box_height*(0.06), 'Roboto Light', extents, font_ext, options)
